...
reservation.post('/post', express.json(), async (req, res) => {
    const { u_id, r_id, e_time, use_equipment, s_time } = req.body;
    var isAlldata = u_id && r_id && e_time && use_equipment && s_time;
    if (isAlldata) {
        try {
            var check = await DB.getReservation(u_id);
            console.log(check);
            if (check.length == 0) {
                var cap = await DB.oneCapacity(r_id);
                var num = await DB.countOverlaps(r_id, s_time, e_time);
                const c = cap[0].capacity;
                console.log(cap[0].capacity);
                console.log(num)
                if (num <= c) {
                    var queryResult = await DB.createReservation(u_id, r_id, s_time, e_time, use_equipment);
                    ...
                    console.log("No space in room")
                    res.status(400).json({
                        "success": false,
                        "message": "No free space at that time"
                    });
                }
            } else {
...
reservation.post('/edit', async (req, res) => {
    const { id, r_id, s_time, e_time } = req.body;
    var isAlldata = id && r_id && s_time && e_time
    if (isAlldata) {
        try {
            var cap = await DB.oneCapacity(r_id);
            var num = await DB.countOverlaps(r_id, s_time, e_time);
            ...
            if (num < c) {
                var queryResult = await DB.editReservation(id, s_time, e_time);
                if (queryResult.affectedRows) {
                    console.log("Registered new user")
                    res.status(201).json({
                        "success": true,
